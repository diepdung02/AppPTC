...
type OutputScreenProps = {
  ...
  route: { params: { productId: number; productName: string; components: Component[] } };
};

const OutputScreen: React.FC<OutputScreenProps> = ({ navigation, route }) => {
  const { productId, productName, components } = route.params;
  ...
  const handleComplete = () => {
    ...
    const completedComponents = selectedComponents.map((componentId) => ({
      id: productId,
      componentIndex: components.findIndex((comp) => comp.id === componentId),
    }));

    navigation.navigate('OutputList', { completedComponents, components });
  };
  ...
  const renderComponentItem = ({ item }: { item: Component }) => {
    ...
    return (
      ...
      >
        ...
        <Text style={styles.componentStatus}>{isSelected ? 'Đã hoàn thành' : 'Chưa hoàn thành'}</Text>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.productInfo}>
        ...
        <FlatList
          data={components.filter(component => !component.isCompleted)}
