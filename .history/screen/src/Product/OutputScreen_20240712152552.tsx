...
import { RootStackParamList } from '../../navigator/navigation'; // Updated import path
...
type OutputScreenProps = {
  ...
  route: { params: { productId: number; components: Component[]; productName: string } };
};
...
const OutputScreen: React.FC<OutputScreenProps> = ({ navigation, route }) => {
  ...
  const handleComplete = () => {
    ...
    const selectedComponentNames = components.filter((component) => selectedComponents.includes(component.id)).map((component) => component.name);
    Alert.alert('Thông báo', `Đã gửi hoàn thành các bộ phận: ${selectedComponentNames.join(', ')}`);
