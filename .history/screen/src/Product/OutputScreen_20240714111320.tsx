...
const OutputScreen = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Lưu thông tin sản phẩm đã hoàn thành vào AsyncStorage
      ...
      // Thêm sản phẩm mới vào danh sách đã hoàn thành
      parsedProducts.push({
        id: productId,
        name: productName,
      ...
      // Xóa danh sách các thành phần đã chọn khỏi state selectedComponents
      setSelectedComponents([]);

      // Thông báo thành công
      Alert.alert("Thông báo", "Đã gửi thông tin thành công.");

      // Chuyển sang màn hình OutputList với các thông tin cần thiết
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() => handleSelectComponent(item)} // Ensure handleSelectComponent is correctly referenced here
          >
