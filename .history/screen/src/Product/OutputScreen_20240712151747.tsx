...
import { RootStackParamList } from '../../navigator/navigation'; // Corrected import path
...
type OutputScreenProps = {
  ...
  route: { params: { productId: number; components: Component[]; name:string; productName: } };
};

const OutputScreen: React.FC<OutputScreenProps> = ({ navigation, route }) => {
  const { productId, components, name } = route.params;
  ...
  const handleComplete = () => {
    ...
    Alert.alert('Thông báo', `Đã gửi hoàn thành các bộ phận: ${selectedComponents.join(', ')}`);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.productInfo}>
        <Text style={styles.productName}>Sản phẩm: {name}</Text>
        <FlatList
          data={components}
