...
const OutputScreen = ({
  ...
  useEffect(() => {
    if (!productName || !components || !productId || !productClient || !productImage || !productPDF || !productCode) {
      Alert.alert(
        "Lỗi dữ liệu",
        "Không thể tải sản phẩm. Vui lòng quay lại và thử lại."
      );
      navigation.goBack(); // Quay lại màn hình trước đó nếu thiếu dữ liệu
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigation.navigate("OutputList", {
        ...
        components,         // Danh sách các thành phần
        productId,          // ID của sản phẩm
        productClient,      // Mã khách hàng
        productImage,       // Đường dẫn hình ảnh
        productPDF,         // Đường dẫn tài liệu PDF
