...
import { RootStackParamList } from '../../navigator/navigation'; // Điều chỉnh đường dẫn import theo cấu trúc dự án của bạn
import FontAwesome from 'react-native-vector-icons/FontAwesome';
import AsyncStorage from '@react-native-async-storage/async-storage';
import COLORS from '../../../constants/Color';
import { Component, Product } from '../../navigator/natigation'; // Giả sử các kiểu Component và Product được định nghĩa trong một file riêng
...
const ProductScreen: React.FC<ProductScreenProps> = ({ navigation }) => {
        ...
        const updatedProducts = products.map((product) => {
          if (product.id === selectedProductId) {
            const updatedComponents = product.components.map((component) => {
              ...
              return { ...component, isCompleted: isSelected };
            });
  ...
  const toggleOutputMenu = (productId: number) => {
    if (selectedProductId === productId) {
      ...
      if (product) {
        setSelectedComponents(product.components.filter((c) => c.isCompleted));
      } else {
        setSelectedComponents([]);
      }
    }
            ...
            {item.components.map((component) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  { backgroundColor: component.isCompleted ? COLORS.primary : 'transparent' },
                ]}
                onPress={() => toggleComponentSelection(component)}>
  ...
  const toggleComponentSelection = (component: Component) => {
    const updatedComponents = selectedComponents.includes(component)
      ? selectedComponents.filter((c) => c.id !== component.id)
