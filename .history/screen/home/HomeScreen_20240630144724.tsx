...
const HomeScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      const userId = '1'; // Replace with actual userId or retrieve dynamically
      const response: ApiResponse<any> = await fetchUser(userId);
      if (response.success) {
        setUserData(response.data); // Assuming the API returns an object with 'name' and 'dept' properties
      } else {
