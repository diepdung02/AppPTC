...
const reports: Report[] = [
  {
    ...
    detail: {
      ...
      lamination: 'Kiểm độ ẩm\nGhép : độ ẩm giữa 2 mối ghép ≤ 2%\nGhép : đường keo\nGhép : nứt/hở',
      machinery: 'Công Đoạn Máy : Lỗ khoan/ lỗ mộng- theo bản vẽ/ rập\nCông Đoạn Máy : Sự vuông góc/ tưa/ cháy',
      carving: 'Chi Tiết Chạm : tuân theo mẫu chạm\nChi Tiết Chạm : Dấu máy/ dấu nhám\nChi Tiết Chạm : Dơ bẩn',
...
const DetailRow = ({ label, value }: { label: string; value: string }) => (
  <View style={tw`flex-row mb-2`}>
    <Text style={tw`font-semibold w-1/3`}>{label}:</Text>
    <Text style={tw`w-2/3`}>{value}</Text>
  </View>
);
  ...
  return (
    <SafeAreaView style={[tw`flex-1`, { backgroundColor: COLORS.colorMain }]}>
      <View style={[tw`flex-row items-center py-2.5 px-5`, { backgroundColor: COLORS.white }]}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('LeaveRequest')} style={tw`p-2`} activeOpacity={0.7}>
      ...
      <ScrollView style={tw`p-4`}>
        {reports.filter(report => report.reportNo.includes(search)).map((report) => (
          <TouchableOpacity
            style={[tw`p-2.5 m-1.25 rounded-md shadow-md`, { backgroundColor: COLORS.white }]}
            key={report.stt}
            onPress={() => handleReportPress(report)}
          >
            <View>
              <Text style={tw`text-lg font-bold mb-2`}>Report No: {report.reportNo}</Text>
              ...
              <DetailRow label="Location/Team" value={report.locationOrTeam} />
              ...
              <DetailRow label="Check & Verify by" value={report.checkAndVerifyBy} />
